...
function updateMetrics(stats) {
    ...
    const metrics = [
        { title: "Загальна сума", value: stats.total.toLocaleString('uk-UA'), unit: currency },
        { title: "Середнє на день", value: stats.avg_per_day.toLocaleString('uk-UA'), unit: currency },
        { title: "Середнє на місяць", value: stats.avg_per_month.toLocaleString('uk-UA'), unit: currency },
        { title: "Кількість транзакцій", value: stats.transaction_count },
        { title: "Найбільша джерело", value: `${stats.top_source} (${stats.top_percent}%)` }
    ];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('apply-custom')?.addEventListener('click', () => {
        ...
        if (customFrom && customTo && customFrom <= customTo) {
            ...
            alert('Вкажіть коректний діапазон дат');
        }
    });
